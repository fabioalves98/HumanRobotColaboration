...
def wrenchCallback(data):
    ...
    for f in int_forces:
        if abs(f) > INT_FORCE and not s_tap:
            s_tap = True
            print("Tap - %s - %s" % (['X', 'Y', 'Z'][int_forces.index(f)], 
                    'Positive' if f > 0 else 'Negative'))
        
        if s_tap == True:
            s_tap_counter +=1
            if s_tap_counter > 200:
                s_tap_counter = 0
                s_tap = False

    # Double Tap Control
    if abs(f_x_int) > INT_FORCE or abs(f_y_int) > INT_FORCE or abs(f_z_int) > INT_FORCE:
        tap = True
