...
pcl::visualization::PCLVisualizer::Ptr viewer_cec;
...
void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    offset.setOrigin(tf2::Vector3(0, 0.25, 0));
    ...
    // Treat every cluster as an obstacle
    for (int i = 0; i < clusters->size(); i++)
    {
    ...
    // viewer_cec->updatePointCloud(cloud_out, "cec_cloud");
    viewer_viz->spinOnce();
    // viewer_cec->spinOnce();
}
...
int main (int argc, char** argv)
{
    ...
    // viewer_cec = normalVis(cloud_global, "3DCECViewer", "cec_cloud");
